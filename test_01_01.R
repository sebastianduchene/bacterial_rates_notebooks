...
tr <- read.tree('tree_01_01.tree')
states <- read.table('par_01_01.csv', head = T, sep = ',')

is_observed <- states[, 2]
names(is_observed) <- states[, 1]

m1 <- run_mcmc(tree = tr, is_observed = is_observed, start_par = c(0.5, 0.5), n_steps = 50000)

pdf('out_par_01_01.pdf')
par(mfrow = c(2, 2))
